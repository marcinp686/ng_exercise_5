...
export class EmployeeService {
  ...
  fakeLocal: Observable<RestApiExampleResponse<Employee[]>> = of(
    {
      status  : "Ok",
      data    : this.fakeData,
      message : "Data fetched no problemo."
    });
    ...
    getEmployees(): Observable<Employee[]> {      
        return this.http.get<RestApiExampleResponse<Employee[]>>('https://dummy.restapiexample.com/api/v1/employees')
          .pipe( map( (resp : RestApiExampleResponse<Employee[]>) => { return resp.data }));       
        //return this.fakeLocal.pipe( map( (response: RestApiExampleResponse<Employee[]>) => { return response.data }  ));
    }
